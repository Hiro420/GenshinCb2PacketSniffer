...
using static Cb2PacketSniffer.PEHeader;
...
public class Misc
{
	...
	public static List<PEHeader.SectionTable> GetSections(PEHeader.DosHeader dosHeader)
	{
		List<PEHeader.SectionTable> ret = new List<PEHeader.SectionTable>();

		// PEHeader.Cleanup(ref dosHeader);
		PEHeader.SectionTable[] sections = dosHeader.section_table!;
		for (int idx = 0; idx < dosHeader.pe.numberOfSections; idx++)
		{
			ret.Add(sections[idx]);
		}
	...
	public static PEHeader.DosHeader GetDosHeader(BinaryReader reader)
	{
		PEHeader.DosHeader dosHeader = new PEHeader.DosHeader();
		PEHeader.ReadDos(reader, ref dosHeader);
		PEHeader.ReadPe(reader, ref dosHeader);
	...
	public static ulong GetOptionalHeader(DosHeader dosHeader)
	{
		return dosHeader.pe.optionalHeader.imageBase;
	}
	...
	public static void PrintExports(ref PEHeader.DosHeader dosHeader)
	{
		...
		{
			ExportAddressName nameAddr = dosHeader.exportDir.exportAddr_name_t![i];
			Console.WriteLine($"   [{i}] {nameAddr.names} Function RVA: 0x{nameAddr.functionRVA:X8} (Ordinal: {nameAddr.ordinal})");
		}
	}

	public static List<ExportAddressName> GetExports(PEHeader.DosHeader dosHeader)
	{
		return dosHeader.exportDir.exportAddr_name_t!.ToList();
	}
