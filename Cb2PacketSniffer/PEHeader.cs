...
public class PEHeader
{
	...
	public static void ReadPe(BinaryReader reader, ref DosHeader dosHeader)
	{
		...
		{
			dosHeader.pe.optionalHeader.imageBase = reader.ReadUInt64();
		}
		else
		{
	...
	{
		uint count = dosHeader.exportDir.numberOfNamePointers;

		uint namePtrTableOffset = (uint)RvaToOffset(dosHeader.pe.numberOfSections,
													dosHeader.exportDir.namePtrRVA,
													dosHeader.section_table!);
		uint ordinalTableOffset = (uint)RvaToOffset(dosHeader.pe.numberOfSections,
													dosHeader.exportDir.ordinalTableRVA,
													dosHeader.section_table!);
		uint eatOffset = (uint)RvaToOffset(dosHeader.pe.numberOfSections,
										   dosHeader.exportDir.exportAddrTableRVA,
										   dosHeader.section_table!);

		dosHeader.exportDir.exportAddr_name_t = new ExportAddressName[count];

		for (uint i = 0; i < count; i++)
		{
			// 1. Read name RVA
			reader.BaseStream.Seek(namePtrTableOffset + i * 4, SeekOrigin.Begin);
			uint nameRVA = Read32LE(reader);
			uint nameOffset = (uint)RvaToOffset(dosHeader.pe.numberOfSections, nameRVA, dosHeader.section_table!);
			...
			// 3. Read ordinal
			reader.BaseStream.Seek(ordinalTableOffset + i * 2, SeekOrigin.Begin);
			ushort ordinal = Read16LE(reader);

			// 4. Lookup function RVA via ordinal
			reader.BaseStream.Seek(eatOffset + ordinal * 4, SeekOrigin.Begin);
			uint functionRVA = Read32LE(reader);

			dosHeader.exportDir.exportAddr_name_t[i] = new ExportAddressName
			{
				names = name,
				nameRVA = nameRVA,
				ordinal = ordinal,
